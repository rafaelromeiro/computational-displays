...
void main() {
    ...
    vec3 pixelLL = vec3(floor(gl_FragCoord.xy) / display2Resolution - 0.5, 1.0) * display2Size;
    vec3 pixelUR = vec3(ceil(gl_FragCoord.xy) / display2Resolution - 0.5, 1.0) * display2Size;
            ...
            for (float deltaS = 0.0; deltaS < MAX_SPATIAL_RESOLUTION; deltaS++) {
                ...
                for (float deltaT = 0.0; deltaT < MAX_SPATIAL_RESOLUTION; deltaT++) {
                    ...
                    if (insideTextureCoordRange(display1Coord)) {
                        vec3 display1Color = texture2D(display1, display1Coord).rgb;
                        ...
                        denominator += tfColor * display1Color;
                    }
                }
            }
        }
